...
int main() {
  ...
  // scene.loadVDB("smoke2.vdb", 0, Vec3f(0.1f, 0.8f, 0.2f), Vec3f(1, 1, 1),
  //               40.0f);
  scene.loadObj("cornellbox_smoke.obj");
  scene.loadVDB("smoke.vdb", 0, Vec3f(0.1f), Vec3f(2, 1, 5), 5.0f);
  scene.build();

  // render
  UniformSampler sampler;
  PathTracingNEE integrator(camera, n_samples, max_depth);
